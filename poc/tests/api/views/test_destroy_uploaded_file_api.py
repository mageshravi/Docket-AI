...
def test_with_anonymous_user(api_client, cases):
    response = api_client.delete(_get_api_url(cases["mahadevan_vs_gopalan"].uuid, 1))
...
def test_with_file_not_associated_with_case(api_client, cases, users, case_factory):
    ...
    response = api_client.delete(_get_api_url(case_1.uuid, other_uploaded_file.id))
...
def test_happy_path(api_client, cases, users):
    ...
    response = api_client.delete(
        _get_api_url(cases["mahadevan_vs_gopalan"].uuid, file_id)
    )
    assert response.status_code == 204

    # Verify that the uploaded file is marked as deleted
    uploaded_file.refresh_from_db()
    assert uploaded_file.is_deleted
    assert uploaded_file.uploaded_file_embeddings.count() == 0
